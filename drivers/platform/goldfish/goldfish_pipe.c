				       ...
				       size_t bufflen, int is_write)
{
	...
	const int cmd = is_write ? CMD_WRITE_BUFFER : CMD_READ_BUFFER;
	const int cmd_wake = is_write ? CMD_WAKE_ON_WRITE : CMD_WAKE_ON_READ;
	...
	while (address < address_end) {
		...
		int status, page_i, num_contiguous_pages;
		...
		for (page_i = 1; page_i < ret; page_i++) {
			xaddr_i = page_to_phys(pages[page_i]) |
				  (address & ~PAGE_MASK);
			if (xaddr_i == xaddr_prev + PAGE_SIZE) {
				...
				pr_err("%s: discontinuous page boundary: %d "
				       "pages instead\n", __func__, page_i);
				break;
			}
		}
		...
		if (access_with_param(dev, cmd, xaddr, avail, pipe, &status)) {
			gf_write_ptr(pipe, dev->base + PIPE_REG_CHANNEL,
				     dev->base + PIPE_REG_CHANNEL_HIGH);
