 ...
 *                        (invariant after initialized)
 * @inner_lock:           inner lock: can nest under outer_lock and/or node lock
 * @outer_lock:           outer lock: no nesting under innor or node lock
			      ...
			      struct binder_proc *proc, int print_all)
{
	...
	binder_inner_proc_lock(proc);
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n))
		print_binder_thread_oilocked(m,
					...
					struct binder_transaction_log_entry *e)
{
	seq_printf(m,
		   ...
		   e->return_error_line);
	/*
	 * read-barrier to synchronize access to e->debug_id_done
	 */
	rmb();
	seq_printf(m, e->debug_id && e->debug_id == e->debug_id_done ?
			"\n" : " (incomplete)\n");
}
