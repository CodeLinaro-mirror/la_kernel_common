					   ...
					   binder_uintptr_t ptr)
{
	...
	binder_inner_proc_lock(proc);
	node = binder_get_node_ilocked(proc, tmp_ref, ptr);
	binder_inner_proc_unlock(proc);
	return node;
}
						    ...
						    binder_uintptr_t cookie)
{
	...
	while (*p) {
		...
		if (ptr < tmp_node->ptr)
			 ...
			 * and return it.
			 */
			binder_init_tmp_ref(tmp_ref, tmp_node);
			hlist_add_head(&tmp_ref->node_entry, &tmp_node->refs);
			return tmp_node;
		}
	}
					   ...
					   binder_uintptr_t cookie)
{
	...
	if (!new_node)
		return NULL;
	binder_inner_proc_lock(proc);
	node = binder_init_node_ilocked(proc, tmp_ref, new_node, ptr, cookie);
	binder_inner_proc_unlock(proc);
	if (node != new_node)
		/*
			    ...
			    struct binder_ref *tmp_ref)
{
	...
	delete_node = binder_dec_node_nilocked(node, 0, 1);
	if (node->proc)
		binder_inner_proc_unlock(node->proc);
	binder_node_unlock(node);
	if (delete_node)
			...
			struct binder_ref_data *rdata)
{
	...
	ret = binder_inc_ref_olocked(ref, strong, wlist);
	BUG_ON(!rdata);
	*rdata = ref->data;
	binder_proc_unlock(proc);
