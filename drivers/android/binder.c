 ...
 * structure is unsafe to access without holding the proc lock.
 */
struct binder_ref {
	/* Lookups needed: */
				   ...
				   int strong, int internal)
{
	...
	if (proc && (node->has_strong_ref || node->has_weak_ref)) {
		...
		if (hlist_empty(&node->refs) && !node->local_strong_refs &&
		    !node->local_weak_refs) {
			if (proc) {
				binder_dequeue_work(proc, &node->work);
...
/**
 * binder_init_ref() - initialize newly allocated ref
 ...
 *		this case.
 */
static struct binder_ref *binder_init_ref(struct binder_proc *proc,
					  struct binder_node *node,
			...
			struct binder_ref_data *rdata)
{
	...
	if (!ref) {
		...
		if (!new_ref)
			return -ENOSPC;
		ref = binder_init_ref(proc, node, new_ref);
	}
	ret = binder_inc_ref(ref, strong, wlist);
	BUG_ON(!rdata);
	*rdata = ref->data;
	...
	while (ptr < end && thread->return_error.cmd == BR_OK) {
		...
		case BC_CLEAR_DEATH_NOTIFICATION: {
			...
			if (cmd == BC_REQUEST_DEATH_NOTIFICATION) {
				...
				death->cookie = cookie;
				/*
				 * When locking is added, we will
				 * have to release the lock to allocate
				 * memory for "death". Therefore we
				 * need to look up the ref again in
			      ...
			      struct binder_proc *proc, int print_all)
{
	...
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
		...
		if (print_all || node->has_async_transaction)
			print_binder_node_locked(m, node);
		binder_node_unlock(node);
	}
	if (print_all) {
