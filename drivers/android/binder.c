 ...
 *                        (protected by proc->inner_lock if node->proc
 *                         and by node->lock)
 ...
 * the node->lock and if node->proc is set, they are also protected
 * by proc->inner_lock to provide serialization with the todo work
 * queue handling. Unless otherwise noted, elements are protected
 * by node->lock.
 */
struct binder_node {
	int debug_id;
 ...
 * structure is unsafe to access without holding the proc outer lock.
 */
struct binder_ref {
	/* Lookups needed: */
...
struct binder_proc {
	...
	struct binder_context *context;
	spinlock_t inner_lock; /* can nest under outer_lock and/or node lock*/
...
_binder_node_inner_unlock(struct binder_node *node, int line)
{
	...
	binder_debug(BINDER_DEBUG_SPINLOCKS,
		     "%s: line=%d\n", __func__, line);
	/*
	 * The node lock is released before the inner lock
	 * since there are paths where the node can be
	 * freed as soon as the inner lock is released
	 */
	spin_unlock(&node->lock);
	if (proc)
...
static bool binder_worklist_empty_ilocked(struct binder_worklist *wlist)
{
				  ...
				  struct binder_worklist *wlist)
{
	...
	ret = binder_worklist_empty_ilocked(wlist);
	binder_inner_proc_unlock(proc);
	return ret;
}
		    ...
		    struct binder_worklist *target_wlist)
{
	...
	binder_enqueue_work_ilocked(work, target_wlist);
	binder_inner_proc_unlock(proc);
}
					...
					struct binder_worklist *wlist)
{
	...
	WARN_ON(proc != wlist->proc);
	binder_inner_proc_lock(proc);
	w = binder_dequeue_work_head_ilocked(wlist);
	binder_inner_proc_unlock(proc);
	return w;
}
				           ...
				           unsigned accept_fds)
{
	...
	if (!new_node)
		return NULL;
	binder_inner_proc_lock(proc);
	node = binder_init_node_ilocked(proc, new_node, ptr, cookie,
				        min_priority, accept_fds);
	binder_inner_proc_unlock(proc);
	if (node != new_node)
		/*
...
static int binder_inc_node(struct binder_node *node, int strong, int internal,
			   int tmp_ref, struct binder_worklist *target_list)
{
	int ret;

	binder_node_inner_lock(node);
	ret = binder_inc_node_nilocked(node, strong, internal,
				       tmp_ref, target_list);
...
static bool binder_dec_node_nilocked(struct binder_node *node,
				     int strong, int internal, int tmp_ref)
{
	struct binder_proc *proc = node->proc;
			    ...
			    int tmp_ref)
{
	...
	binder_node_inner_lock(node);
	free_node = binder_dec_node_nilocked(node, strong, internal, tmp_ref);
	binder_node_inner_unlock(node);
	if (free_node)
...
static void binder_cleanup_ref(struct binder_ref *ref)
{
	...
	if (ref->data.strong)
		binder_dec_node_nilocked(ref->node, 1, 1, 0);

	hlist_del(&ref->node_entry);
	delete_node = binder_dec_node_nilocked(ref->node, 0, 1, 0);
			  ...
			  struct binder_worklist *target_list)
{
	...
	if (strong) {
		if (ref->data.strong == 0) {
			ret = binder_inc_node(ref->node, 1, 1, 0, target_list);
			if (ret)
				return ret;
		}
		ref->data.strong++;
		...
		if (ref->data.weak == 0) {
			ret = binder_inc_node(ref->node, 0, 1, 0, target_list);
			if (ret)
				return ret;
		}
		ref->data.weak++;
...
static bool binder_dec_ref(struct binder_ref *ref, int strong)
{
	if (strong) {
		...
		ref->data.strong--;
		if (ref->data.strong == 0)
			binder_dec_node(ref->node, strong, 1, 0);
	} else {
		if (ref->data.weak == 0) {
					      ...
					      binder_size_t *failed_at)
{
	...
	if (buffer->target_node)
		binder_dec_node(buffer->target_node, 1, 0, 0);
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		if (tr->target.handle) {
			...
			binder_inc_node(target_node, 1, 0, 0, NULL);
			mutex_unlock(&context->context_mgr_node_lock);
		}
...
err_no_context_mgr_node:
	...
	if (target_node)
		binder_dec_node(target_node, 1, 0, 0);
			      ...
			      struct binder_thread *thread)
{
	...
	while (t) {
		...
		if (t->to_thread == thread && !t->to_thread_invalid) {
			...
			t = t->to_parent;
		} else if (t->from == thread && !t->from_invalid) {
			t->from_invalid = 1;
			t = t->from_parent;
		} else
			break;
...
static int binder_ioctl_set_ctx_mgr(struct file *filp)
{
	...
	new_node = binder_new_node(proc, 0, 0, 0, 0);
	if (!new_node) {
		ret = -ENOMEM;
		goto out;
	}
			      ...
			      struct binder_proc *proc, int print_all)
{
	...
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
		...
		if (print_all || node->has_async_transaction)
			print_binder_node_nilocked(m, node, false, true);
	}
	binder_inner_proc_unlock(proc);
	if (print_all) {
...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(node, &binder_dead_nodes, dead_node) {
		binder_node_lock(node);
		print_binder_node_nilocked(m, node, true, false);
		binder_node_unlock(node);
	}
	mutex_unlock(&binder_dead_nodes_lock);
