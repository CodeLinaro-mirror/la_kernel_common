...
static int binder_apply_fd_fixups(struct binder_transaction *t)
{
	...
	list_for_each_entry(fixup, &t->fd_fixups, fixup_entry) {
		...
		*fdp = fd;
		/*
		 * Some architectures require the kernel address
		 * to be flushed before the address can be
		 * accessed by userspace at a different virtual
		 * address (eg. VIVT cache)
		 */
		flush_dcache_page(virt_to_page(fdp));
		fixup->file = NULL;
	}
	list_for_each_entry_safe(fixup, tmp, &t->fd_fixups, fixup_entry) {
		if (fixup->file) {
