 ...
 * via binder_dequeue_work(). Each proc has a list_lock used for
 * synchronization.
 */
...
{
	WARN_ON(!spin_is_locked(&wlist->proc->list_lock));
				  ...
				  struct binder_worklist *wlist)
{
	...
	spin_lock(&proc->list_lock);
	ret = binder_worklist_empty_locked(wlist);
	spin_unlock(&proc->list_lock);
	return ret;
}

/**
 * binder_proc_lock() - Acquire spinlock for given binder_proc
 * @proc:         struct binder_proc to acquire
 *
 * Acquires proc->proc_lock. Used to protect binder_proc,
 * binder_node, binder_ref structures associated with the
 * given proc.
 */
#define binder_proc_lock(_proc) _binder_proc_lock(_proc, __LINE__)
static void
				  ...
				  struct binder_worklist *target_list)
{
	...
	BUG_ON(!spin_is_locked(&node->node_lock));
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
		...
		if (proc && !node->has_strong_ref && target_list) {
			spin_lock(&proc->list_lock);
						  ...
						  struct binder_node *node)
{
	 ...
	 * recompute the insertion point and possibly
	 * abort if the ref was added by another thread.
	 */
...
static void binder_delete_ref(struct binder_ref *ref)
{
	...
	binder_proc_lock(ref->proc);
	rb_erase(&ref->rb_node_desc, &ref->proc->refs_by_desc);
	rb_erase(&ref->rb_node_node, &ref->proc->refs_by_node);
	binder_proc_unlock(ref->proc);

	if (ref->strong)
		binder_dec_node(ref->node, 1, 1);

	spin_lock(&ref->node->node_lock);
	hlist_del(&ref->node_entry);
	spin_unlock(&ref->node->node_lock);
...
static int binder_dec_ref(struct binder_ref *ref, int strong)
{
	...
	bool dec_node = false;
	struct binder_proc *proc = ref->proc;
				   ...
				   struct binder_thread *thread)
{
	...
	if (ref->node->proc == target_proc) {
		...
		binder_inc_node_locked(ref->node,
				fp->hdr.type == BINDER_TYPE_BINDER,
				0, NULL);
			       ...
			       struct binder_transaction *in_reply_to)
{
	...
	if (in_reply_to)
		...
		spin_lock(&t->buffer->target_node->node_lock);
		target_allows_fd = t->buffer->target_node->accept_fds;
		spin_unlock(&t->buffer->target_node->node_lock);
	}
	if (!target_allows_fd) {
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		spin_lock(&target_node->node_lock);
		target_proc = target_node->proc;
		if (target_proc == NULL) {
			spin_unlock(&target_node->node_lock);
			return_error = BR_DEAD_REPLY;
			return_error_line = __LINE__;
			goto err_dead_binder;
		}
	...
	if (reply) {
		...
		binder_proc_lock(target_node->proc);
		spin_lock(&target_node->node_lock);
		 ...
		 */
		binder_enqueue_work(target_proc, &t->work, target_list);
		binder_proc_unlock(target_node->proc);
	}
	if (target_wait) {
		if (reply || !(tr->flags & TF_ONE_WAY))
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error.cmd == BR_OK) {
		...
		case BC_ACQUIRE_DONE: {
			...
			free_node = binder_dec_node_locked(node,
			...
			if (buffer->async_transaction && buffer->target_node) {
				...
				if (!w)
					...
					binder_enqueue_work_locked(
							w, &thread->todo);
				spin_unlock(&proc->list_lock);
				spin_unlock(&buf_node->node_lock);
			}
...
static void print_binder_transaction_locked(struct seq_file *m,
					    const char *prefix,
					    struct binder_transaction *t)
{
	seq_printf(m,
				     ...
				     struct binder_work *w)
{
	...
	WARN_ON(!spin_is_locked(&w->wlist->proc->list_lock));

	switch (w->type) {
	case BINDER_WORK_TRANSACTION:
		t = container_of(w, struct binder_transaction, work);
		print_binder_transaction_locked(m, transaction_prefix, t);
		break;
	case BINDER_WORK_RETURN_ERROR: {
		struct binder_error *e = container_of(
				w, struct binder_error, work);
				       ...
				       int print_always)
{
	...
	WARN_ON(!spin_is_locked(&thread->proc->proc_lock));
	...
	while (t) {
		if (t->from == thread) {
			print_binder_transaction_locked(m,
				     ...
				     struct binder_node *node)
{
	...
	WARN_ON(!spin_is_locked(&node->node_lock));
	...
	seq_puts(m, "\n");
	if (node->proc) {
		spin_lock(&node->proc->list_lock);
		list_for_each_entry(w, &node->async_todo.list, entry)
			print_binder_work_locked(m, "    ",
					  "    pending async transaction", w);
		spin_unlock(&node->proc->list_lock);
	}
}
...
static void print_binder_ref_locked(struct seq_file *m,
				    struct binder_ref *ref)
{
	WARN_ON(!spin_is_locked(&ref->proc->proc_lock));
	spin_lock(&ref->node->node_lock);
