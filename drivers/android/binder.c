...
static bool binder_worklist_empty(struct binder_worklist *wlist)
{
	...
	spin_lock(&wlist->proc->list_lock);
	ret = binder_worklist_empty_locked(wlist);
	spin_unlock(&wlist->proc->list_lock);
	return ret;
}
...
binder_enqueue_work(struct binder_work *work,
		    struct binder_worklist *target_wlist)
{
	spin_lock(&target_wlist->proc->list_lock);
	binder_enqueue_work_locked(work, target_wlist);
	spin_unlock(&target_wlist->proc->list_lock);
}
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_FREE_BUFFER: {
			...
			if (buffer->async_transaction && buffer->target_node) {
				...
				spin_lock(&buf_node->proc->list_lock);
				...
				if (!w)
					...
					binder_enqueue_work_locked(
							w, &thread->todo);
				spin_unlock(&buf_node->proc->list_lock);
				binder_proc_unlock(buf_node->proc);
			}
...
retry:
	...
	while (1) {
		...
		if (!w && wait_for_proc_work) {
			if (!binder_worklist_empty_locked(&proc->todo)) {
...
static void binder_release_work(struct binder_worklist *wlist)
{
	...
	while (1) {
		w = binder_dequeue_work_head(wlist);
		if (!w)
			return;
