					      ...
					      binder_size_t *failed_at)
{
	...
	for (offp = off_start; offp < off_end; offp++) {
		...
		case BINDER_TYPE_FD: {
			...
			/*
			 * If in context of transaction target, then
			 * the fd fixups have been applied and we
			 * need to close the fd
			 */
			if (failed_at && proc->tsk == current->group_leader)
				sys_close(fp->fd);
		} break;
		case BINDER_TYPE_PTR:
			/*
...
static int binder_apply_fd_fixups(struct binder_transaction *t)
{
	...
	list_for_each_entry(fixup, &t->fd_fixups, fixup_entry) {
		...
		if (fd < 0) {
			binder_debug(BINDER_DEBUG_TRANSACTION,
				     "failed fd fixup txn %d fd %d -> %d\n",
				     t->debug_id, *fixup->fdp, fd);
			ret = -ENOMEM;
			break;
		}
		binder_debug(BINDER_DEBUG_TRANSACTION,
			     "fd fixup txn %d fd %d -> %d\n",
			     t->debug_id, *fixup->fdp, fd);
		trace_binder_transaction_fd(t, *fixup->fdp, fd);
