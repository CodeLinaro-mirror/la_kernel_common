 ...
 *                        (protected by proc->inner_lock while node->proc
 *                        is valid, and by binder_dead_nodes_lock and
 *                        node->lock when node->proc is NULL)
...
static void print_binder_transaction_olocked(struct seq_file *m,
					     struct binder_proc *proc,
					     const char *prefix,
					     struct binder_transaction *t)
{
				     ...
				     struct binder_work *w)
{
	...
	case BINDER_WORK_TRANSACTION:
		t = container_of(w, struct binder_transaction, work);
		print_binder_transaction_olocked(
				         ...
				         int print_always)
{
	...
	while (t) {
		if (t->from == thread) {
			print_binder_transaction_olocked(m, thread->proc,
...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	/*
	 * Print the dead nodes. Since we need the dead_nodes_lock
	 * to traverse the nodes, but need the node lock to print them, we
	 * need to create a temporary array of nodes while holding
	 * the dead nodes lock and then print the nodes using the array
	 * while holding the node locks.
	 */
	spin_lock(&binder_dead_nodes_lock);
