...
void init_sched_energy_costs(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (of_find_property(cn, "freq_energy_model", NULL))
...
static int sched_energy_probe(struct platform_device *pdev)
{
	...
	int max_cpu_uarch_scale = INT_MIN;
	int cpu;
	...
	for_each_possible_cpu(cpu) {
		...
		int cpu_uarch_scale = topology_get_cpu_uarch_scale(cpu);

		max_cpu_uarch_scale = max(cpu_uarch_scale, max_cpu_uarch_scale);
	...
	for_each_possible_cpu(cpu) {
		...
		cpu_max_cap = DIV_ROUND_UP(cpu_max_cap * cpu_uarch_scale,
					   max_cpu_uarch_scale);
		...
		if (sge_l0 && sge_l0->nr_cap_states > 0) {
			...
			dev_info(&pdev->dev,
				"cpu=%d uarch_scale=%d [freq=%ld cap=%ld power_d0=%ld] -> [freq=%ld cap=%ld power_d0=%ld]\n",
				cpu, cpu_uarch_scale,
		...
		dev_dbg(&pdev->dev,
			"cpu=%d uarch_scale=%d max_frequency=%ld max_uarch_scale=%d cpu_max_capacity=%ld\n",
			cpu, cpu_uarch_scale, max_frequencies[cpu], max_cpu_uarch_scale,
			cpu_max_cap);
